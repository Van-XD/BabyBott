    ...
    try {
        if (/shop|toko/i.test(command)) {
            ...
            case 'buy':
                switch (_type) {
                    case 'potion':
                        ...
                        break
                    case 'limit':
        		    if (global.DATABASE._data.users[m.sender].money >= limit * count) {
		        	global.DATABASE._data.users[m.sender].money -= limiy * count
				global.DATABASE._data.users[m.sender].limit += count * 1
				conn.reply(m.chat, `Succes membeli ${count} Limit dengan harga ${limit * count} money\n\nGunakan limit untuk mengakses command`,m)
			    } else conn.reply(m.chat, `Uang amda tidak cukup untuk membeli ${count} Limit dengan harga ${limit * count} money`,)
			    break
                    case 'diamond':
                ...
                switch (_type) {
                    ...
                    case 'limit':
                        if (global.DATABASE._data.users[m.sender].limit >= count * 1) {
                            global.DATABASE._data.users[m.sender].money += limiy * count
                            global.DATABASE._data.users[m.sender].limit -= count * 1
                            conn.reply(m.chat, `Succes menjual ${count} Limit dengan harga ${Slimit * count} money`.trim(), m)
                        } else conn.reply(m.chat, `Limit kami tidak cukup`.trim(),m)
                        break
                    case 'common':
            ...
            switch (type) {
                ...
                case 'limit':
		        if (global.DATABASE._data.users[m.sender].money >= limit * count) {
			    global.DATABASE._data.users[m.sender].money -= limit * count
			    global.DATABASE._data.users[m.sender].limit += count * 1
			    conn.reply(m.chat, `Succes membeli ${count} Limit dengan harga ${limit * count} money\n\nGunakan limit untuk mengakses command`,m)
		        } else conn.reply(m.chat, `Uang amda tidak cukup untuk membeli ${count} Limit dengan harga ${limit * count} money`, m)
		    break
                case 'diamond':
                        if (global.DATABASE._data.users[m.sender].money >= Bdiamond * count) {
                            global.DATABASE._data.users[m.sender].diamond += count * 1
