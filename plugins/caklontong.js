...
let handler  = async (m, { conn, usedPrefix }) => {
    ...
    let res = await fetch('https://leyscoders-api.herokuapp.com/api/caklontong?apikey=MIMINGANZ')
    let json = await res.json()
    conn.caklontong[id] = [
      await conn.reply(m.chat, `Soal: *${json.result.soal}*\nTimeout: *${(timeout / 1000).toFixed(2)} detik*\nKetik *${usedPrefix}clhint* untuk hint\nBonus: ${poin} XP`, m),
      json, poin,
      setTimeout(() => {
        if (conn.caklontong[id]) conn.reply(m.chat, `Waktu habis!\nJawabannya : *${json.result.jawaban}*`, conn.caklontong[id][0])
