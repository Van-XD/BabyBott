      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (!isPrems && plugin.limit && global.DATABASE._data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `Limit anda habis, silahkan beli di shop dengan ketik */buy*\nAtau minta sama owner dengan ketik */owner*`, m)
      ...
      case 'remove':
        if (chat.welcome) {
          ...
          for (let user of participants) {
            let pp = 'https://i.ibb.co/0fNZzhj/IMG-20210815-WA0033.jpg'
            try {
